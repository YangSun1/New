...
model = dict(
    ...
    test_cfg=dict(
        ...
        stride=(512, 512)),
    init_cfg=None)
dataset_type = 'MotovisDataset'
data_root = 'data/seg_data'
img_norm_cfg = dict(
    mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True)
train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='LoadMotovisNPZ', crop_height_start=480, crop_height_end=980),
    dict(
        type='MotovisTransform',
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        ...
        custom_keys=dict(
            decode_head=dict(lr_mult=2.0), pixel_decoder=dict(lr_mult=2.0))))
...
evaluation = dict(
    interval=5000, metric=['mIoU', 'mDice'], pre_eval=True, save_best='mIoU')
crop_size = (896, 896)
pretrained = 'pretrained/beitv2_large_patch16_224_pt1k_ft21k.pth'
work_dir = './work_dirs/vit_ada_motovis'
gpu_ids = range(0, 2)
auto_resume = False
device = 'cuda'
